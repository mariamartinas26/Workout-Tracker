...
RETURN QUERY
    WITH exercise_stats AS (
        ...
        SELECT
            ...
            --computes avg weight for an exercise
            COALESCE(AVG(wel.weight_used_kg), 0) as avg_weight_used,
            --computes avg number of repetitions for an exercise
            COALESCE(AVG(wel.reps_completed), 0) as avg_reps,
            --computes avg number of completed sets for an exercise
            COALESCE(AVG(wel.sets_completed), 0) as avg_sets,
            --calculates avg difficulty rate, if rating does not exist=>3
